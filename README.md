...
> ⚠️ **모니터링 필요**: 약 100건마다 IROS 사이트가 응답하지 않거나 브라우저가 멈출 수 있습니다. 터미널을 주기적으로 확인하고, 멈추면 브라우저를 닫고 이어서 실행하세요:
> ```bash
> python3 iros_cart.py config.json 100  # 100번째부터 이어서
> ```
...
> ⚠️ **모니터링 필요**: 장바구니와 마찬가지로 약 100건마다 브라우저가 멈출 수 있습니다. 멈추면 브라우저를 닫고 다시 실행하세요. 이미 다운로드된 파일은 폴더에 남아있으므로 중복 다운로드 걱정 없습니다.
