...
1. 결제할 항목(최대 10건씩) 선택 → 결제 버튼
2. 카드 정보 입력(카드번호, 유효기간, CVC, 비밀번호)
3. 결제 승인

결제가 끝나면 **다시 터미널로 돌아가서 Enter**. 브라우저가 자동으로 닫힙니다.

> 여러 번 끊어 결제(예: 30건을 10건씩 3회)하려면, 10건 결제 → 새로고침 → 다음 10건 순으로 브라우저에서 반복하세요. 터미널 Enter는 전체 결제가 다 끝난 뒤 한 번만 누릅니다.
