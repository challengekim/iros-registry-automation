...
| 공동인증서 / 간편인증 | 로그인 시 사용 |
| 카드 | 결제(수동) 시 필요. **법인: 10만원 미만 일괄 / 부동산: 페이지당 10건** |
| Python 3.10 이상 | |
...
```
1) 상호명/법인등록번호 준비 → 2) 장바구니 → 3) 결제(수동, 10만원 미만/회) → 4) 열람·저장
```
...
```
1) 주소/동호수 JSON 준비 → 2) 장바구니 → 3) 결제(수동, 페이지당 10건) → 4) 일괄열람출력
```
...
```
  ★ 결제대상: 1건 - 로그인 상태에서 한 번에 10만원 미만까지 일괄 결제 (법인)
...
1. 결제할 항목 선택 → 결제 버튼 (법인: 10만원 미만 일괄, 회원 로그인 필수)
2. 카드 정보 입력(카드번호, 유효기간, CVC, 비밀번호)
3. 결제 승인

결제가 끝나면 **다시 터미널로 돌아가서 Enter**. 브라우저가 자동으로 닫힙니다.

> 법인은 10만원 미만 단위로 결제합니다. 금액 초과 시 여러 회 나눠 결제 → 새로고침 → 나머지 순으로 반복하세요. 터미널 Enter는 전체 결제가 다 끝난 뒤 한 번만 누릅니다.
...
| "결제대상: N건" 안내 | 브라우저 | 카드 결제 (법인: 10만원 미만 일괄) |
...
- IROS 결제는 반드시 사람이 직접 진행합니다. 법인: 로그인 상태에서 10만원 미만 일괄 / 부동산: 페이지당 최대 10건씩 일괄 열람.
