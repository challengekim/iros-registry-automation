...
### Step 1: 법인등록번호 / 상호명 준비

장바구니 자동화에 필요한 입력 파일을 준비합니다.

#### 이미 법인등록번호 또는 상호명 목록이 있는 경우

엑셀이나 다른 시스템에서 이미 법인등록번호 또는 상호명을 확보하고 있다면, 직접 입력 파일을 만들어 **Step 2로 바로 진행**할 수 있습니다.

**법인등록번호 목록** (`data/iros_corpnums.json`):
```json
{
...
**상호명 목록** (`data/iros_companies.json`):
```json
["스마트솔루션", "디지털마케팅", "클라우드서비스"]
```

#### 사업자등록번호만 있는 경우 — Bizno 조회

사업자등록번호(10자리)만 있고 법인등록번호/상호명을 모르는 경우, bizno.net에서 자동 조회할 수 있습니다.
