...
> **주 용도는 법인등기부등본 대량 발급**입니다. 법인은 **장바구니 담기·결제 후 열람·저장 모두 자동 권장**이며 결제만 수동입니다(**페이지당 10건** 제한). 수백~수천 건 발급 시 자동화 효용이 가장 큽니다. ⭐ **10건 이상 한 번에 결제하는 방법을 아시는 분은 제보 부탁드려요** (이메일 / GitHub Issues — 하단 [문의](#문의) 참고). (단, Windows에서 IROS 일괄열람이 동작하는 환경이라면 수동도 추천)
>
> **부동산은 장바구니 담기만 자동을 권장하고, 결제·열람·다운로드는 수동이 권장됩니다.** IROS 웹 UI에서 **로그인 시 10만원 미만 일괄 결제**와 **일괄열람출력**(한 페이지에 뜬 건들을 한 번에 저장)이 기본 제공되므로, 결제·열람 단계는 브라우저에서 수동으로 처리하는 편이 더 빠르고 안전합니다. 주소 리스트가 많아 반복 검색·담기가 번거로울 때만 부동산 장바구니 자동화를 쓰세요.
...
### 법인등기부등본 (장바구니·열람·저장 자동 권장, 결제만 수동)

```
1) 상호명/법인등록번호 준비 → 2) 장바구니(자동) → 3) 결제(수동, 페이지당 10건/회) → 4) 열람·저장(자동)
```
...
- Windows 환경에서 IROS 일괄열람이 정상 동작하는 경우에는 열람·저장 단계도 수동으로 처리하는 것이 간편할 수 있습니다.

### 부동산등기부등본 (장바구니만 자동 권장, 결제·열람·다운로드는 수동 권장)

```
1) 주소/동호수 JSON 준비 → 2) 장바구니(자동) → 3) 결제(수동, 10만원 미만 일괄) → 4) 일괄열람출력/일괄저장(수동)
```

- 장바구니: `iros_cart_realty.py` — 주소 리스트가 많을 때 자동화 효용이 큽니다.
- 열람·저장: `iros_download_realty.py` (필요 시에만 사용)
- IROS 웹에서 **로그인 시 10만원 미만 일괄 결제**와 **일괄열람출력·일괄저장** 버튼이 기본 제공되므로, 결제·열람 단계는 브라우저에서 수동으로 처리하는 편이 빠르고 안전합니다.
- 자동 장바구니 담기가 필요하지 않을 정도로 건수가 적다면 전 단계를 수동으로 진행해도 됩니다.
